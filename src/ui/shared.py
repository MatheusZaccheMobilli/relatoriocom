...
<style>
    ...
    /* Linha guia vertical de 100% na barra de progresso */
    .mob-meta-bar::after {
        content: "";
        position: absolute;
        top: 0; bottom: 0;
...
def aplicar_css(em_construcao: bool = True) -> None:
    ...
    Se `em_construcao=False`, oculta o banner amarelo (mantém a faixa
    preta do header). O gap vazio entre header e conteúdo é removido
    em ambos os modos.
    """
    css = CSS_MOBILLI
    # Sempre sobe o conteúdo pra encostar no header preto, em ambos os modos
    css = css + (
        "<style>"
